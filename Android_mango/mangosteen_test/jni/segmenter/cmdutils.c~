...
{
    const char *opt, *arg;
    int optindex, handleoptions=1, i;
    const OptionDef *po;
    ...
    while (optindex < argc) {
        ...
        if (handleoptions && opt[0] == '-' && opt[1] != '\0') {
            ...
            if (!po->name) {
unknown_opt:
				__android_log_print(ANDROID_LOG_DEBUG, "test", "unrecognized option '%s'\n", opt);
            ...
            if (po->flags & HAS_ARG) {
                ...
                if (!arg) {
				__android_log_print(ANDROID_LOG_DEBUG, "test", "%s: missing argument for option '%s'\n", argv[0], opt);
            ...
            if (po->flags & OPT_STRING) {
                ...
                *po->u.float_arg = parse_number_or_die(opt, arg, OPT_FLOAT, -INFINITY, INFINITY);
	            ...
	            __android_log_print(ANDROID_LOG_DEBUG, "test", "failed to set value '%s' for option '%s'\n", arg, opt);
		...
		{
		__android_log_print(ANDROID_LOG_DEBUG, "test", "po->flags OPT_EXIT");
                exit(0);
		}
        } else {
		...
		{
		__android_log_print(ANDROID_LOG_DEBUG, "test", "parse_arg_function(NULL, opt) < 0");
                    exit(1);
		}
            }
        }
...
{
    fprintf(stderr, "%s version , Copyright (c) %d-%d the FFmpeg developers\n",
