...
int main(int argc, char **argv)
{
    ...
    if(nb_output_files <= 0 && nb_input_files == 0) {
        show_usage();
		__android_log_print(ANDROID_LOG_DEBUG, "test", "Use -h to get full help or, even better, run 'man ffmpeg");
        fprintf(stderr, "Use -h to get full help or, even better, run 'man ffmpeg'\n");
        ffmpeg_exit(1);
    }
    /* file converter / grab */
    if (nb_output_files <= 0) {
		__android_log_print(ANDROID_LOG_DEBUG, "test", "At least one output file must be specified");
        fprintf(stderr, "At least one output file must be specified\n");
        ffmpeg_exit(1);
    }
    if (nb_input_files == 0) {
		__android_log_print(ANDROID_LOG_DEBUG, "test", "At least one input file must be specified");
