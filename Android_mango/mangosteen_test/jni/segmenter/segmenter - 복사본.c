...
#define PKT_FLAG_KEY AVMEDIA_TYPE_VIDEO
...
int Segmenter(int argc, char argv[6][100])
{
	...
	__android_log_print(ANDROID_LOG_DEBUG, "test", argv[2]);
	__android_log_print(ANDROID_LOG_DEBUG, "test", "segment_duration : %lf", segment_duration);
    ...
    *dot = '.';
    ...
    video_index = -1;
    audio_index = -1;
	__android_log_print(ANDROID_LOG_DEBUG, "test", "1");
    ...
    do {
		...
		__android_log_print(ANDROID_LOG_DEBUG, "test", "packet.stream_index : %d, video_index : %d", packet.stream_index, video_index);
        if (packet.stream_index == video_index && (packet.flags & PKT_FLAG_KEY)) {
            segment_time = (double)video_st->pts.val * video_st->time_base.num / video_st->time_base.den;
			__android_log_print(ANDROID_LOG_DEBUG, "test", "if1");
        }
        else if (video_index < 0) {
            segment_time = (double)audio_st->pts.val * audio_st->time_base.num / audio_st->time_base.den;
			__android_log_print(ANDROID_LOG_DEBUG, "test", "if2");
        }
        else {
            segment_time = prev_segment_time;
        }
		//tempchar[0]=(char)segment_time;
		//tempchar[1]=(char)prev_segment_time;
		//tempchar[2]=(char)segment_duration;;
		//tempchar[3]='\0';
		//__android_log_print(ANDROID_LOG_DEBUG, "test", "segment_time :%lf, prev_segment_time : %lf, segment_duration : %lf", segment_time, prev_segment_time, segment_duration);
		//__android_log_print(ANDROID_LOG_DEBUG, "test", "123");
        if (segment_time - prev_segment_time >= segment_duration) {
            ...
            if (write_index) {
                write_index = !write_index_file(index, tmp_index, segment_duration, output_prefix, http_prefix, first_segment, ++last_segment, 0, max_tsfiles);
				__android_log_print(ANDROID_LOG_DEBUG, "test", "2.1");
            }
...
int Java_net_jbong_FFmpegBasic_FFmpegBasic_TSsegmenter(JNIEnv *env, jobject thiz, jobjectArray filePath)
{
	//const jbyte *str;
	//int result;
	char str[6][100];
