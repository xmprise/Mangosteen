...
(function($) {
  ...
  $.fn.extend($,{   
    ...
    Dav: function(res) {
      ...
      if(isString(res)) {
        /* Change to below by ERICKAHN
        resourceUrl     = res;
        */
        resourceUrl     = encodeURI(res);        
        resource        = {};
        //;;console.log("request uri : " + res + " -> " + resourceUrl);
      }
      else {
        resource          = res;
      ...
      var api = function() {
         ...
         */
        this.copy = function(cob) {
            //;;;console.log('== COPY ==');
            //var dUrl = cob.Destination_url;
            var dUrl = encodeURI(cob.Destination_url);
            //;;;console.log('dUrl =' +dUrl);
            this.prepare(cob, 'COPY');
            extendBeforeSend(cob, function(xhr) {
              xhr.setRequestHeader('Destination', dUrl);
              xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');			  
              xhr.setRequestHeader('Overwrite', cob.Overwrite);			  
          });			
         ...
         * @href    http://greenbytes.de/tech/webdav/rfc4918.html#METHOD_COPY
         */
        this.move = function(cob) {
            //;;;console.log('== Move ==');
            //var dUrl = cob.Destination_url;
            var dUrl = encodeURI(cob.Destination_url);
            //;;;console.log('dUrl =' +dUrl);
            this.prepare(cob, 'MOVE');
            extendBeforeSend(cob, function(xhr) {
              xhr.setRequestHeader('Destination', dUrl);
              xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
              xhr.setRequestHeader('Overwrite', cob.Overwrite);
        ...
        this.rename = function(cob) {
              ...
              */
              var dUrl = encodeURI(cob.Destination_url);
              //;;;console.log('dUrl =' +dUrl);
              this.prepare(cob, 'RENAME');
              extendBeforeSend(cob, function(xhr) {
              xhr.setRequestHeader('Allow-rename', dUrl);
              xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');		
          });			
        ...
        this.lock = function(cob) {          
          ...
          if(user) {
            ...
            extendBeforeSend(cob, function(xhr) {
              xhr.setRequestHeader('Timeout', hdrs.Timeout);
          });            
