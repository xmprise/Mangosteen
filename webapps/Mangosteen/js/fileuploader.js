...
qq.FileUploaderBasic.prototype = {
    ...
    _isAllowedExtension: function(fileName){
    	...
    	if( $('.mediaType').attr('id') == 'image')
    	{
    		if(!(/.gif$/).test(fileName.toLowerCase()) && !(/.png$/).test(fileName.toLowerCase()) && !(/.jpg$/).test(fileName.toLowerCase())&& !(/.jpeg$/).test(fileName.toLowerCase()))
    			return false;
    	}
...
qq.extend(qq.UploadHandlerXhr.prototype, {
    ...
    _upload: function(id, params){
        ...
        queryString = queryString+"&mediaType="+mediaType+"&uploadPath="+uploadPath;  // ³»°¡ Ãß°¡ÇÑ Äõ¸®ºÎºÐ
        var protocol = this._options.demoMode ? "GET" : "POST";     
...
//        xhr.setRequestHeader("X-File-Name", encodeURIComponent(name));
        ...
        file.name = name;   
        console.log("111");
        if( $('.mediaType').attr('id') == 'image' || $('.mediaType').attr('id') == 'video' || $('.mediaType').attr('id') == 'audio')
		{
			xhr.setRequestHeader("X-File-UploadPath", $('.mediaType').attr('folderPath'));
			file.uploadPath =$('.mediaType').attr('folderPath'); 
			uploadPath = $('.mediaType').attr('folderPath'); 
			xhr.setRequestHeader("X-File-Type", $('.mediaType').attr('id'));
			file.mediaType = $('.mediaType').attr('id');
			mediaType =  $('.mediaType').attr('id');
			
			console.log("22>>>"+$('.mediaType').attr('folderPath'));
			console.log("33>>>"+$('.mediaType').attr('id'));
			
		}
		else
		{
			xhr.setRequestHeader("X-File-UploadPath", $('#MainFileView ul').attr('path'));
			file.uploadPath = $('#MainFileView ul').attr('path');
			uploadPath = $('#MainFileView ul').attr('path');
		}
        ...
        for (key in this._options.customHeaders){
        	alert(key);
        	alert(this._options.customHeaders[key]);
    ...
    _onComplete: function(id, xhr){
        ...
        if (xhr.status == 200){
        	...
        	{ 
            	...
            	//console.log(xhr.responseText);
            	mangoAPI.updateAddData(xhr.responseText);
        	}
        	else
        	{
        		 mangoAPI.FileExplorerUpdateAddData(xhr.responseText);
        	}    
